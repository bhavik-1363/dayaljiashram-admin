...
class FirebaseVerificationService implements VerificationService {
  ...
  async getMemberVerificationRequests(
    memberId: string
  ): Promise<VerificationRequest[]> {
    try {
      const verificationRef = collection(db, "verification_requests");
  ...
  async createVerificationRequest(
    ...
    try {
      ...
      Object.keys(changes).forEach((key) => {
        ...
        if (
          changes[typedKey] !== undefined &&
          JSON.stringify(changes[typedKey]) !== JSON.stringify(member[typedKey])
        ) {
